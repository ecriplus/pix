...
module('Integration | Component | Places::Statistics', function (hooks) {
  ...
  test('it should display occupied and available places', async function (assert) {
    ...
    assert.ok(screen.getAllByText(t('cards.available-seats-count.value', { total: model.total }))[0]);
    assert.ok(screen.getByText(model.occupied));
    assert.ok(screen.getAllByText(t('cards.occupied-seats-count.value', { total: model.total }))[1]);
    assert.notOk(screen.queryByText(t('cards.occupied-seats-count.anonymous', { count: model.anonymousSeat })));
  });
  ...
  test('it should display anonymous seat informations', async function (assert) {
    ...
    // then
    assert.ok(screen.getByText(t('cards.occupied-seats-count.anonymous', { count: model.anonymousSeat })));
  });
});
