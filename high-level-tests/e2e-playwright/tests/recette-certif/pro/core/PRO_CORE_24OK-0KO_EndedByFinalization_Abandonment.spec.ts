...
const testRef = 'PRO_CORE_24OK-0KO_EndedByFinalization_Abandonment';
...
test.describe(testRef, () => {
  ...
  test(
    `${testRef} - User takes a certification test for a PRO certification center, only CORE subscription. answers until 25th question. Ended by finalization for abandonment.`,
    {
      tag: ['@snapshot'],
      annotation: [
        {
    ...
    async ({ page: pixAppPage, preparedCertificationTest, pixSuperAdminContext, snapshotHandler }) => {
      ...
      await test.step('User checks their certification result', async () => {
        ...
        expect(pixScoreObtained).toEqual('PIX 808 CERTIFIÉS');
