...
test.describe(testRef, () => {
  ...
  test(
    `${testRef} - User takes a certification test for a PRO certification center, only CORE subscription. one challenge only answered. Ended by invigilator for abandonment`,
    {
      tag: ['@snapshot'],
      annotation: [
        {
    ...
    async ({ page: pixAppPage, preparedCertificationTest, pixSuperAdminContext }) => {
      ...
      const { sessionNumber, invigilatorOverviewPage, pixCertifPage, snapshotHandler } = preparedCertificationTest;
