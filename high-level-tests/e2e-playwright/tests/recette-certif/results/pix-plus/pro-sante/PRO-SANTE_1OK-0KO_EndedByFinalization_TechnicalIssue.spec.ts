...
const testRef = 'PRO-SANTE_1OK-0KO_EndedByFinalization_TechnicalIssue';
const snapshotPath = `recette-certif/${testRef}/${testRef}.json`;

test(
  `${testRef} - User takes a certification test for a Pix+ Pro Santé subscription. One challenge only answered. Ended by finalization for technical issue`,
  {
    tag: ['@snapshot'],
    annotation: [
      {
