...
const testRef = 'DROIT_1OK-0KO_EndedByFinalization_TechnicalIssue';
const snapshotPath = `recette-certif/${testRef}/${testRef}.json`;

test(
  `${testRef} - User takes a certification test for a Pix+ Droit subscription. One challenge only answered. Ended by finalization for technical issue`,
  {
    tag: ['@snapshot'],
    annotation: [
      {
