...
- Rendez votre test capable de, ou bien enregistrer le snapshot, ou bien vérifier le snapshot
...
```js
test('[@snapshot] this is a snapshot test', async ({}, testInfo) => {
  testInfo.annotations.push(
    {
      type: 'tag',
...
TEST_MODE=record npx playwright test --grep @snapshot
