...
async function list({ locale } = {}) {
  ...
  return areaDataObjects.sort(byId).map((areaData) => _toDomain({ areaData, locale }));
}
...
async function findByFrameworkIdWithCompetences({ frameworkId, locale }) {
  ...
  const areas = areaDatas.sort(byId).map((areaData) => _toDomain({ areaData, locale }));
...
async function findByFrameworkId({ frameworkId, locale }) {
  const areaDatas = await areaDatasource.findByFrameworkId(frameworkId);
  return areaDatas.sort(byId).map((areaData) => _toDomain({ areaData, locale }));
}

async function findByRecordIds({ areaIds, locale }) {
  const areaDataObjects = await areaDatasource.list();
  return areaDataObjects
    .filter(({ id }) => areaIds.includes(id))
    .sort(byId)
    .map((areaData) => _toDomain({ areaData, locale }));
}
